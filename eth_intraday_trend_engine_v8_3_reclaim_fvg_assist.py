...
"""
ETH INTRADAY TREND ENGINE v8.3 RECLAIM-FIRST + FVG ASSIST
- ETH 전용 기본값 (환경변수로 다른 심볼도 가능)
- 당일 청산
- reclaim 중심 / pullback 보조 / FVG continuation 보조 / 하루 0~4회 지향
...
- Engine C + F:
  ...
  Engine F = bullish FVG revisit continuation (long-only, conservative)
"""
...
ALLOW_LONG_FVG = os.getenv("ALLOW_LONG_FVG", "1").strip() == "1"
ALLOW_SHORT_FVG = os.getenv("ALLOW_SHORT_FVG", "0").strip() == "1"
...
# Engine F (bullish FVG assist, conservative long-only)
FVG_MIN_GAP_PCT = float(os.getenv("FVG_MIN_GAP_PCT", "0.10"))
FVG_MAX_GAP_PCT = float(os.getenv("FVG_MAX_GAP_PCT", "0.65"))
FVG_MAX_AGE_BARS = int(os.getenv("FVG_MAX_AGE_BARS", "18"))
FVG_MIN_DISPLACEMENT_ATR = float(os.getenv("FVG_MIN_DISPLACEMENT_ATR", "0.55"))
FVG_VOLUME_RATIO_MIN = float(os.getenv("FVG_VOLUME_RATIO_MIN", "1.00"))
FVG_REVISIT_DEPTH_MAX = float(os.getenv("FVG_REVISIT_DEPTH_MAX", "0.85"))
FVG_REQUIRE_1H_STRENGTH = os.getenv("FVG_REQUIRE_1H_STRENGTH", "1").strip() == "1"
FVG_REQUIRE_15M_TREND = os.getenv("FVG_REQUIRE_15M_TREND", "1").strip() == "1"
FVG_REQUIRE_VWAP_ALIGN = os.getenv("FVG_REQUIRE_VWAP_ALIGN", "1").strip() == "1"
FVG_REQUIRE_REACC = os.getenv("FVG_REQUIRE_REACC", "1").strip() == "1"
FVG_PREVENT_OVERLAP_REENTRY = os.getenv("FVG_PREVENT_OVERLAP_REENTRY", "1").strip() == "1"
...
def add_features_5m(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["range_abs"] = (out["high"] - out["low"]).abs()
    out["bull_fvg_bottom_create"] = out["high"].shift(2)
    out["bull_fvg_top_create"] = out["low"]
    out["bull_fvg_gap_pct_create"] = safe_series_div(
        out["bull_fvg_top_create"] - out["bull_fvg_bottom_create"],
        out["bull_fvg_bottom_create"]
    ) * 100.0
    displacement_ok = safe_series_div(out["body_abs"].shift(1), out["atr14"].shift(1)).fillna(0.0) >= FVG_MIN_DISPLACEMENT_ATR
    vol_ok = safe_series_div(out["volume"].shift(1), out["vol_ma20"].shift(1)).fillna(0.0) >= FVG_VOLUME_RATIO_MIN
    out["bull_fvg_create"] = (
        (out["bull_fvg_bottom_create"].notna()) &
        (out["bull_fvg_top_create"].notna()) &
        (out["bull_fvg_bottom_create"] < out["bull_fvg_top_create"]) &
        (out["bull_fvg_gap_pct_create"] >= FVG_MIN_GAP_PCT) &
        (out["bull_fvg_gap_pct_create"] <= FVG_MAX_GAP_PCT) &
        displacement_ok & vol_ok
    ).astype(int)

    out["active_bull_fvg_bottom"] = np.nan
    out["active_bull_fvg_top"] = np.nan
    out["active_bull_fvg_mid"] = np.nan
    out["active_bull_fvg_age"] = np.nan
    out["active_bull_fvg_gap_pct"] = np.nan
    out["bull_fvg_revisit_depth"] = np.nan
    out["in_active_bull_fvg"] = 0

    active_bottom = np.nan
    active_top = np.nan
    active_gap_pct = np.nan
    active_age = None
    active_touched = False

    for idx in range(len(out)):
        if out.iloc[idx]["bull_fvg_create"] == 1:
            active_bottom = float(out.iloc[idx]["bull_fvg_bottom_create"])
            active_top = float(out.iloc[idx]["bull_fvg_top_create"])
            active_gap_pct = float(out.iloc[idx]["bull_fvg_gap_pct_create"])
            active_age = 0
            active_touched = False
        elif active_age is not None:
            active_age += 1

        if active_age is None or active_age > FVG_MAX_AGE_BARS:
            active_bottom = np.nan
            active_top = np.nan
            active_gap_pct = np.nan
            active_age = None
        ...
        if active_age is not None and pd.notna(active_bottom) and pd.notna(active_top):
            low_i = float(out.iloc[idx]["low"])
            high_i = float(out.iloc[idx]["high"])
            overlap = (low_i <= active_top) and (high_i >= active_bottom)
            revisit_depth = 0.0
            if overlap and active_top > active_bottom:
                revisit_depth = max(0.0, min(1.5, (active_top - low_i) / (active_top - active_bottom)))
            out.iat[idx, out.columns.get_loc("active_bull_fvg_bottom")] = active_bottom
            out.iat[idx, out.columns.get_loc("active_bull_fvg_top")] = active_top
            out.iat[idx, out.columns.get_loc("active_bull_fvg_mid")] = (active_top + active_bottom) / 2.0
            out.iat[idx, out.columns.get_loc("active_bull_fvg_age")] = float(active_age)
            out.iat[idx, out.columns.get_loc("active_bull_fvg_gap_pct")] = active_gap_pct
            out.iat[idx, out.columns.get_loc("bull_fvg_revisit_depth")] = revisit_depth
            out.iat[idx, out.columns.get_loc("in_active_bull_fvg")] = int(overlap)
            if overlap:
                if FVG_PREVENT_OVERLAP_REENTRY and active_touched:
                    active_bottom = np.nan
                    active_top = np.nan
                    active_gap_pct = np.nan
                    active_age = None
...
def signal_engine_f_long_fvg(df: pd.DataFrame, i: int) -> Tuple[bool, str, Dict[str, float]]:
    if not ALLOW_LONG or not ALLOW_LONG_FVG:
        return False, "LONG_DISABLED", {}
    if i < 1:
        return False, "WARMUP", {}
    row = df.iloc[i]
    ...
    if regime == "NO_LONG":
        return False, "REGIME_FAIL", {"regime_4h": regime}
    if row.get("bull_fvg_create", 0) == 1:
        return False, "FVG_SAMEBAR_CREATE", {"regime_4h": regime}
    if row.get("in_active_bull_fvg", 0) != 1:
        return False, "FVG_NOT_IN_ZONE", {"regime_4h": regime}
    fvg_bottom = float(row["active_bull_fvg_bottom"]) if pd.notna(row["active_bull_fvg_bottom"]) else np.nan
    fvg_top = float(row["active_bull_fvg_top"]) if pd.notna(row["active_bull_fvg_top"]) else np.nan
    fvg_mid = float(row["active_bull_fvg_mid"]) if pd.notna(row["active_bull_fvg_mid"]) else np.nan
    if pd.isna(fvg_bottom) or pd.isna(fvg_top) or pd.isna(fvg_mid):
        return False, "FVG_NO_LEVEL", {"regime_4h": regime}
    if FVG_REQUIRE_1H_STRENGTH and row["close_1h"] < row["ema20_1h"] * (1.0 - PULLBACK_1H_ALLOWANCE_PCT):
        return False, "FVG_FAIL_1H", {"regime_4h": regime}
    trend_15m = (row["close_15m"] >= row["ema20_15m"] * 1.000) if FVG_REQUIRE_15M_TREND else True
    slope_ok = row["ema20_slope_bps_15m"] >= max(C_MIN_15M_EMA_SLOPE_BPS, 0.3)
    vwap_ok = (row["close_15m"] >= row["vwap20_15m"] * 1.000) if FVG_REQUIRE_VWAP_ALIGN else True
    age_ok = row["active_bull_fvg_age"] <= FVG_MAX_AGE_BARS
    depth = float(row["bull_fvg_revisit_depth"]) if pd.notna(row["bull_fvg_revisit_depth"]) else 9.0
    depth_ok = 0.0 < depth <= FVG_REVISIT_DEPTH_MAX
    rejection_ok = (row["close"] >= fvg_mid) or ((row["close"] >= row["open"]) and (row["close"] >= fvg_bottom))
    wick_reject_ok = row["low"] <= fvg_top and row["close"] > row["low"] * 1.0008
    prev_break = row["close"] >= prev["high"] * 0.9998
    reacc_ok = c_reacc_confirm_long(df, i) if FVG_REQUIRE_REACC else True
    close_from_mid_pct = safe_div(abs(row["close"] - fvg_mid), fvg_mid) * 100.0
    not_too_far = close_from_mid_pct <= max(C_MAX_CLOSE_FROM_LEVEL_PCT, 0.45)
    if not (trend_15m and slope_ok and vwap_ok and age_ok and depth_ok and rejection_ok and wick_reject_ok and prev_break and reacc_ok and not_too_far):
        return False, "FVG_NO_LONG", {"regime_4h": regime, "close_from_level_pct": float(close_from_mid_pct)}
    meta = {
        "regime_4h": regime,
        "level": fvg_mid,
        "close_from_level_pct": float(close_from_mid_pct),
        "setup_type": "BULLISH_FVG_REVISIT_LONG",
        "engine": "F",
        "fvg_bottom": fvg_bottom,
        "fvg_top": fvg_top,
        "fvg_age": float(row["active_bull_fvg_age"]),
        "fvg_gap_pct": float(row["active_bull_fvg_gap_pct"]) if pd.notna(row["active_bull_fvg_gap_pct"]) else np.nan,
        "fvg_revisit_depth": depth,
    }
    return True, "ALLOW_LONG_FVG", meta
...
def choose_signal(signals: List[Tuple[str, str, Dict[str, float]]]) -> Optional[Tuple[str, str, Dict[str, float]]]:
    ...
    priority = {
        ...
        "ALLOW_LONG_FVG": 2,
        "ALLOW_LONG_C_PULLBACK": 3,
        "ALLOW_SHORT_C_PULLBACK": 4,
    }
...
def save_outputs(
    ...
    lines = ["ETH INTRADAY TREND ENGINE v8.3 RECLAIM-FIRST + FVG ASSIST", "=" * 60]
